...
from core.mixins import acesso_orcamentos, orcamento_e_paraiso
...
# Caminho do logo/mascote da Paraíso — mesmo arquivo usado em pdf_paraiso.py (pedido)
LOGO_PATH_PARAISO = os.path.join(settings.BASE_DIR, 'static', 'img', 'mascote_paraiso.png')


def _draw_footer_logo_paraiso(canvas, doc):
    """Desenha o logo/mascote da Paraíso, fixo no canto inferior direito —
    mesma posição/tamanho do PDF de pedido, sem QR code."""
    canvas.saveState()

    if os.path.exists(LOGO_PATH_PARAISO):
        logo_size = 31 * mm
        logo_x    = PAGE_W - MARGIN - logo_size
        logo_y    = MARGIN - 2 * mm

        canvas.drawImage(
            LOGO_PATH_PARAISO, logo_x, logo_y,
...
@acesso_orcamentos
def orcamento_pdf_paraiso(request, pk):
    orcamento = get_object_or_404(
        Orcamento.objects.select_related('cliente', 'criado_por'),
        pk=pk
    )

    if not orcamento_e_paraiso(orcamento):
        raise PermissionDenied('Este orçamento não pertence à Paraíso do Acrílico.')
    ...
    doc    = SimpleDocTemplate(
        ...
        topMargin=MARGIN,  bottomMargin=MARGIN + 30 * mm,
        title=f"Orçamento #{orcamento.numero}", author='Paraíso do Acrílico',
    )
    ...
    # ── Cabeçalho ──
    left_col = [
        Paragraph("PARAÍSO DO ACRÍLICO", s['company']),
        Spacer(1, 2 * mm),
        Paragraph("Paraiso dos Decoradores LTDA.", s['company_sub']),
        Paragraph("Rua Rosa Umehara Manabe, 444 - Jardim Chácara Mea, Suzano - SP, 08696-470", s['company_sub']),
        Paragraph("CNPJ: 48.282.247/0001-74", s['company_sub']),
        Paragraph("WhatsApp: (11) 96572-5888", s['company_sub']),
    ]
    right_col = [
    ...
    doc.build(el, onFirstPage=_draw_footer_logo_paraiso, onLaterPages=_draw_footer_logo_paraiso)

    pdf = buffer.getvalue()
    buffer.close()

    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'inline; filename="orcamento_{orcamento.numero}_paraiso.pdf"'
